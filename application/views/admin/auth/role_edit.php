...
<article class="cl pd-20">
    <form action="<?= site_url('auth/roleEdit') ?>" method="post" class="form form-horizontal" id="form-admin-add">
        <input type="hidden" name="id" value="<?= $item->id ?>">
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>角色名：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="<?= $item->rolename ?>" placeholder="" id="adminName"
                       name="rolename">
            </div>
        </div>
                                ...
                                <?php if ($v['pid'] == 0): ?>
                                    <dl class="cl permission-list2">
                                        <dt>
                                            <label class="">
                                                <input type="checkbox"
                                                       value="<?= $v['id'] ?>" <?= isset($v['checked']) && $v['checked'] ? 'checked' : '' ?>
                                                       name="menu[]"/>
                                                <?= $v['title'] ?></label>
                                        </dt>
                                        <dd>
                                            <?php foreach ($menus as $kk => $vv): ?>
                                                <?php if ($vv['pid'] == $v['id']): ?>
                                                    <label class="">
                                                        <input <?= isset($vv['checked']) && $vv['checked'] ? 'checked' : '' ?>
