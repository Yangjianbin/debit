        ...
        <article class="cl pd-20">
            ...
            <div class="mt-20">
                <table class="table table-border table-bordered table-bg table-hover table-sort">
                    ...
                    <tr class="text-c">
                        <!--<th>dateId</th>
                        <th>toDayDebitMoney</th>
                        <th>todayShouldRepayMoney</th>
                        <th>toDayRepayMoney</th>
                        <th>toDayOverdueMoney</th>
                        <th>todayOverdueInterest</th>
                        <th>todayExtendInterest</th>
                        <th>todayTotalIncome</th>
                        <th>Operation</th>-->
...
<script type="text/javascript">
    ...
    var table = $('.table-sort').dataTable({
        ...
        "aoColumnDefs": [],
        ajax: "<?=site_url('export/daily')?>",
        columns: [
            {data: "dateId"},
            {data: "toDayDebitMoney", "render": function ( data) {
                    return   number(parseInt(data) / 10000);
                }},
            {data: "returnDebitMoney", "render": function ( data) {
                    return   number(parseInt(data) / 10000);
                }},
            {data: "returnInterest", "render": function ( data) {
                    return   number(parseInt(data) / 10000);
                }},
            {data: "todayExtendInterest", "render": function ( data) {
                    return   number(parseInt(data) / 10000);
                }},
            {data: "todayExtendInterest", "render": function ( data,type,all) {
                    return   number((parseInt(all.returnDebitMoney) + parseInt(all.returnInterest) + parseInt(all.todayExtendInterest)) / 10000);
                }},
            {data: "todayExtendInterest", "render": function ( data,type,all) {
                    return   number((parseInt(all.returnDebitMoney) + parseInt(all.returnInterest) + parseInt(all.todayExtendInterest) - parseInt(all.toDayDebitMoney)) / 10000);
                }},
            {data: "todayExtendMoney", "render": function ( data) {
                    return   number(parseInt(data) / 10000);
                }},
            {data: "toDayOverdueMoney", "render": function ( data) {
                    return   number(parseInt(data) / 10000);
                }},
            {data: "toDayOverdueMoney", "render": function ( data,type,all) {
                    var a = (parseInt(all.returnDebitMoney) + parseInt(all.returnInterest) + parseInt(all.todayExtendInterest)) / 10000
                    var b = parseInt(all.toDayDebitMoney) / 10000;
                    if(!a || !b){
                        return '';
                    }
            ...
            {data: "toDayOverdueMoney", "render": function ( data,type,all) {
                    var a= (parseInt(all.returnDebitMoney) + parseInt(all.returnInterest) + parseInt(all.todayExtendInterest) - parseInt(all.toDayDebitMoney)) / 10000;
                    var b = parseInt(all.toDayDebitMoney) / 10000;
                    if(!a || !b){
                        return '';
                    }
