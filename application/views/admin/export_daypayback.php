            ...
            <div class="mt-20">
                <table class="table table-border table-bordered table-bg table-hover table-sort">
                    ...
                    <tr class="text-c">
                        ...
                        <th>今日审核中人数</th>-->
                        <th>Date</th>
                        <th>No.of Due Today</th>
                        <th>No.of Payback</th>
                        <th>No.of Extend</th>
                        <th>Daily Pendding</th>
                        <th>Payback%</th>
                        <th>Extend%</th>
                        <th>Total Payback%</th>
                        <th>No.of Overdue</th>
                        <th>No.of Auditing</th>
                    </tr>
                    </thead>
                </table>
            </div>
...
<script type="text/javascript">
    ...
    var table = $('.table-sort').dataTable({
        ...
        "aoColumnDefs": [],
        ajax: "<?=site_url('export/daypayback')?>",
        columns: [
            {data: "dateId"},
            {data: "shouldPayBackCount", "render":function (data, type, all) {
                var url = "<?=site_url('export/daypayback_sub')?>" + "/" + all.dateId;
                    return '<a href="'+url+'" style="text-decoration:underline">'+data+'</a>';
                }},
            {data: "paybackUserCount"},
            {data: "extendUserCount"},
            {data: "notPaybackUserCount"},
            {data: "notPaybackUserCount", "render": function ( data,type,all) {
                    var str = 100 * all.paybackUserCount / all.shouldPayBackCount;
                    return str.toFixed(2) + '%';
                }},
            {data: "notPaybackUserCount", "render": function ( data,type,all) {
                    var str = 100 * all.extendUserCount / all.shouldPayBackCount;
                    return str.toFixed(2) + '%';
                }},
            {data: "notPaybackUserCount", "render": function ( data,type,all) {
                    var str = 100 * (parseInt(all.paybackUserCount) + parseInt(all.extendUserCount)) / all.shouldPayBackCount;
                    return str.toFixed(2) + '%';
                }},
            {data: "overdueUserCount"},
            {data: "auditingCount"}
        ],
