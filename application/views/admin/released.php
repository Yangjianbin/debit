    ...
    <div class="Hui-article">
        <article class="cl pd-20">
            ...
            <div class="cl pd-5 bg-1 bk-gray mt-20">
                ...
                <div class="l pd-5">
                    <input type="text" placeholder="ReleaseLoanTime" id="start_time" style="width: 200px;" onfocus="WdatePicker()"
                           class="input-text Wdate"/>
                    -
                    <input type="text" placeholder="ReleaseLoanTime" id="end_time" style="width: 200px;" onfocus="WdatePicker()"
                           class="input-text Wdate"/>
                </div>
                <div class="l pd-5">
                    <input type="text" placeholder="PaybackTime" id="paybacktimeStart" style="width: 200px;" onfocus="WdatePicker()"
                           class="input-text Wdate"/>
                    -
                    <input type="text" placeholder="PaybackTime" id="paybacktimeEnd" style="width: 200px;" onfocus="WdatePicker()"
                           class="input-text Wdate"/>
                </div>
...
<script type="text/javascript">
    ...
    var table = $('.table-sort').dataTable({
        ...
        fnServerParams: function (aoData) {
            ...
            aoData.paybacktimeStart = $('#paybacktimeStart').val();
            aoData.paybacktimeEnd = $('#paybacktimeEnd').val();
        },
        "createdRow": function (row, data, dataIndex) {
            $(row).children('td').attr('style', 'text-align: center;')
        },
        "aoColumnDefs": [],
        ajax: "<?=site_url('admin/released')?>",
        columns: [
            {data: "DebitId"},
            // {data: "DebitMoney"},
            // {data: "DebitPeroid"},
...
//                    return '';
    ...
    function check(id) {
        layer_show('审核', "<?=site_url('admin/overdueCheck/')?>" + id, null, null);
    }
    ...
    function export1() {
        ...
        var paybacktimeStart = $('#paybacktimeStart').val();
        var paybacktimeEnd = $('#paybacktimeEnd').val();
        var startTime = $('#start_time').val();
        var endTime = $('#end_time').val();
        location.href = "<?=site_url('export/released')?>" + '?start_time='
            + startTime + '&end_time=' + endTime + '&userid=' + userid + '&debitid=' + debitid
            + '&phone='+phone + '&idcard='+idcard + '&paybacktimeStart='+paybacktimeStart+'&paybacktimeEnd='+paybacktimeEnd;
    }
</script>
<!--/请在上方写此页面业务相关的脚本-->
